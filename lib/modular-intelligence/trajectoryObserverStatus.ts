...
export function readRuntimeObserverStatus(cwd = process.cwd()) {
  ...
  return {
    ...
    VERIFIED_review_state: rows.filter((r) => (r.review_state as string) === 'VERIFIED').length,
    CURRICULUM_CANDIDATE: rows.filter((r) => (r.review_state as string) === 'CURRICULUM_CANDIDATE').length,
