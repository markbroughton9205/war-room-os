...
export function getIntelligenceSourceRegistry(): IntelligenceSourceDefinition[] {
  return [
    ...
    },
    {
      ...
      cost_level: 'medium',
      configured: configured('GOOGLE_WEB_SEARCH_API_KEY') && configured('GOOGLE_WEB_SEARCH_CLIENT_ID'),
      allowed_use_cases: ['general_research', 'income_generation', 'market_research', 'local_awareness', 'contradiction_analysis'],
      rate_limits: { requests: 30, windowSeconds: 60 },
      failure_behavior: 'degrade',
    },
    {
      ...
      capabilities: ['broad_web_search', 'feed_monitoring'],
      verified_level: 'semi_verified',
      freshness_window: 'minutes-to-days depending on upstream engine',
      reliability_score: 0.72,
      cost_level: 'free',
      configured: configured('SEARXNG_BASE_URL'),
      allowed_use_cases: ['general_research', 'income_generation', 'market_research', 'local_awareness', 'contradiction_analysis'],
      rate_limits: { requests: 30, windowSeconds: 60 },
      failure_behavior: 'degrade',
    },
    {
      ...
      label: 'War Room Local Corpus',
      category: 'verified_structured',
      source_type: 'crawl',
      capabilities: ['page_extraction', 'broad_web_search'],
      verified_level: 'semi_verified',
      freshness_window: 'last successful crawl timestamp',
    ...
    // Bridged lib/research-engine/ providers (Build #4A) — each is credential-checked by its own
    // adapter (returns an honest not_configured failure if the required env is absent), so
    // `configured` here mirrors that same check rather than duplicating provider-specific logic.
    {
      source_id: 'arxiv',
      label: 'arXiv',
      category: 'verified_structured',
      source_type: 'search',
      capabilities: ['broad_web_search', 'official_records'],
      verified_level: 'verified',
      freshness_window: 'preprint publication date',
      reliability_score: 0.8,
      cost_level: 'free',
      configured: true,
      allowed_use_cases: ['general_research', 'market_research', 'contradiction_analysis'],
      rate_limits: { requests: 1, windowSeconds: 3 },
      failure_behavior: 'skip',
    },
    {
      source_id: 'crossref',
      ...
      capabilities: ['broad_web_search', 'official_records'],
      verified_level: 'verified',
      freshness_window: 'publication date',
      reliability_score: 0.8,
      cost_level: 'free',
      configured: true,
      allowed_use_cases: ['general_research', 'market_research', 'contradiction_analysis'],
    ...
    },
    {
      ...
      category: 'verified_structured',
      source_type: 'government_public_data',
      capabilities: ['official_records', 'broad_web_search'],
      verified_level: 'verified',
      freshness_window: 'filing date',
      reliability_score: 0.88,
      cost_level: 'free',
      configured: configured('SEC_EDGAR_USER_AGENT_BASE'),
      allowed_use_cases: ['market_research', 'public_policy', 'supply_chain', 'contradiction_analysis'],
      rate_limits: null,
      failure_behavior: 'degrade',
    },
    {
      ...
      capabilities: ['broad_web_search', 'official_records'],
      verified_level: 'semi_verified',
      freshness_window: 'last-edited date',
      reliability_score: 0.7,
      cost_level: 'free',
      configured: configured('WIKIMEDIA_USER_AGENT_BASE'),
      allowed_use_cases: ['general_research', 'public_policy', 'contradiction_analysis'],
      rate_limits: null,
      failure_behavior: 'skip',
    },
    {
