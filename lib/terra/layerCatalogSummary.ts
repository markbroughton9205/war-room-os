...
export const TERRA_LAYER_SUMMARIES: TerraLayerSummary[] = [
  ...
  { id: 'ny511_cameras', label: 'Traffic Cameras (New York State)', domain: 'other', kind: 'traffic_camera', description: 'New York State traffic camera stills within a bounding box — New York only. Stills via Url; VideoUrl is a Commander link-out only (never scraped or auto-embedded). Attribution "powered by 511NY"; product name is never "511NY" and never implies NYSDOT endorsement. Throttle 10 requests / 60s.', refreshIntervalMs: 60_000 },
  { id: 'caltrans_cwwp2_cameras', label: 'Traffic Cameras (Caltrans CWWP2 — California)', domain: 'other', kind: 'traffic_camera', description: 'Caltrans CWWP2 public CCTV stills within a bounding box — California only (districts d1–d12 intersecting the view). PUBLIC_NO_AUTH status/image files. inService false → OFFLINE. streamingVideoURL is link-out only — ≥10 concurrent streams need a written Caltrans agreement; this layer never opens a stream.', refreshIntervalMs: 60_000 },
