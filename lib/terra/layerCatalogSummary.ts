...
export const TERRA_LAYER_SUMMARIES: TerraLayerSummary[] = [
  ...
  { id: 'ohgo_cameras', label: 'Traffic Cameras (OHGO / ODOT — Ohio)', domain: 'other', kind: 'traffic_camera', description: 'ODOT / OHGO refreshed JPEG cameras within a bounding box — Ohio only (not Ohio Turnpike). PUBLIC_KEY via server-side OHGO_API_KEY. Catalog uses ETag/If-None-Match; LargeUrl is never hammered across the inventory. Catalog freshness is UNAVAILABLE without image Last-Modified — never fabricated LIVE from the poll clock.', refreshIntervalMs: 60_000 },
  { id: 'caltrans_cctv', label: 'Traffic Cameras (Caltrans CWWP2 — California)', domain: 'other', kind: 'traffic_camera', description: 'Caltrans CWWP2 JPEG stills within a bounding box — California only. Keyless per-district JSON. HLS streams are never fetched or embedded. Catalog freshness is UNAVAILABLE without a per-still capture timestamp — never fabricated LIVE from the poll clock.', refreshIntervalMs: 60_000 },
  { id: 'ohgo_events', label: 'Traffic Events (OHGO / ODOT — Ohio)', domain: 'other', kind: 'traffic_event', description: 'ODOT / OHGO incidents, construction, dangerous slowdowns, travel delays, and digital signs within a bounding box — Ohio only. PUBLIC_KEY via server-side OHGO_API_KEY. Empty bbox is LIVE_EMPTY, never fabricated events.', refreshIntervalMs: 60_000 },
  { id: 'ohgo_road_weather', label: 'Road Weather (OHGO / ODOT — Ohio)', domain: 'weather', kind: 'road_weather_observation', description: 'ODOT / OHGO weather sensor sites (RWIS) within a bounding box — Ohio only. PUBLIC_KEY via server-side OHGO_API_KEY.', refreshIntervalMs: 60_000 },
  ...
  { id: 'nasa_eonet_volcanoes', label: 'Active Volcanic Activity (NASA EONET)', domain: 'hazards', kind: 'volcano_event', description: 'Currently active volcanic events via NASA EONET (Smithsonian Global Volcanism Program). PARTIAL coverage — not a global USGS volcano alert-level system.', refreshIntervalMs: 10 * 60 * 1000 },
  ...
  { id: 'nws_severe_weather_alerts', label: 'Severe Weather Alerts (NWS)', domain: 'hazards', kind: 'severe_weather_alert', description: 'NOAA/NWS CAP tornado and severe-weather alerts (Tornado Warning/Watch, Severe Thunderstorm Warning/Watch, Flash Flood Warning, Extreme Wind Warning). Watches are not confirmed tornadoes. Real polygon warning areas where NWS supplies geometry — zone-only alerts are skipped, never guessed.' },
