...
import { buildOhgoStillUrl, parseOhgoStillPath } from '@/lib/terra/ohgoStillPath'
import { ohgoAuthHeaders, ohgoGetJson, ohgoMapBoundsParams, OHGO_API_BASE } from './ohgo_shared'
...
export function mapOhgoCamerasToDocuments(
  ...
  for (const camera of cameras) {
    ...
    usableViews.forEach((view, viewIndex) => {
      ...
      const rawImageUrl = (view.LargeUrl ?? view.largeUrl ?? view.SmallUrl ?? view.smallUrl ?? '').trim() || null
      const imagePath = rawImageUrl ? parseOhgoStillPath(rawImageUrl) : null
      const imageUrl = imagePath ? buildOhgoStillUrl(imagePath) : null
      if (!imageUrl || !imagePath) return
      // Catalog poll does not HEAD LargeUrl (don't hammer stills). Without Last-Modified,
      // freshness is UNAVAILABLE — never fabricated LIVE from the poll clock.
      const freshnessState = resolveTerraTrafficCameraFederationHealth({
        feedType: 'refreshed_image',
        refreshIntervalSec: OHGO_REFRESH_INTERVAL_SEC,
        capturedAtIso: null,
        nowIso: now,
      ...
      rememberCameraImageUrl(PROVIDER, federatedId, imageUrl, ATTRIBUTION)
      ...
      documents.push(makeDocument({
        ...
        identifiers: {
          ...
          ...(imageUrl ? { imageUrl } : {}),
          imagePath,
          collectionIntervalSec: String(OHGO_REFRESH_INTERVAL_SEC),
...
async function search(query: ResearchQuery) {
  ...
  const headers = ohgoAuthHeaders(key, catalogStore.etag)
  const boundedUrl = `${OHGO_API_BASE}/cameras?${ohgoMapBoundsParams({ lamin, lomin, lamax, lomax })}`
  const result = await safeProviderFetch(PROVIDER, boundedUrl, { timeoutMs: 20_000, headers })
  ...
  catalogStore.cameras = cameras
  catalogStore.etag = result.etag
  const documents = mapOhgoCamerasToDocuments(cameras, { lamin, lomin, lamax, lomax }, nowIso(), limit)
  const response = okResponse(PROVIDER, { documents, durationMs: Date.now() - started })
  cacheSet(cacheKey, response, CACHE_TTL.liveFeed)
  return { ok: true as const, response }
}
...
async function healthCheck(): Promise<ResearchHealthStatus> {
  ...
  try {
    const result = await ohgoGetJson(PROVIDER, 'cameras', ohgoApiKey(), { lamin: 41.0, lomin: -81.7, lamax: 41.2, lomax: -81.3 })
