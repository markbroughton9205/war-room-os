...
export const RESEARCH_PROVIDER_ENV: ResearchProviderEnvDescriptor[] = [
  ...
  { id: 'github', displayName: 'GitHub', category: 'code', capabilities: ['search'], requiredEnv: ['GITHUB_TOKEN'], optionalEnv: [], defaultBaseUrls: {}, implemented: true, pendingWhenUnconfigured: false, notes: 'Read-only repository search only this phase (`GET /search/repositories`) — no code/issue/PR/commit/release search is implemented. No Commander base-URL override exists; https://api.github.com is the only official host.' },
  { id: 'sam_gov', displayName: 'SAM.gov', category: 'government_contracts', capabilities: ['search'], requiredEnv: ['SAM_GOV_API_KEY'], optionalEnv: [], defaultBaseUrls: {}, implemented: false, pendingWhenUnconfigured: false, notes: 'Opportunities v2 read-only discovery. Not yet implemented as a Research Engine adapter this build phase (a separate revenue-focused adapter exists in lib/opportunity-agents/sources).' },
  { id: 'fmcsa', displayName: 'FMCSA', category: 'transportation', capabilities: ['search', 'getById'], requiredEnv: ['FMCSA_WEB_KEY'], optionalEnv: [], defaultBaseUrls: {}, implemented: false, pendingWhenUnconfigured: false, notes: 'Carrier/safety lookup. Adapter not yet implemented this build phase.' },
  ...
  { id: 'semantic_scholar', displayName: 'Semantic Scholar', category: 'scholarly', capabilities: ['search', 'getById', 'citations', 'related'], requiredEnv: [], optionalEnv: ['SEMANTIC_SCHOLAR_API_KEY'], defaultBaseUrls: {}, implemented: false, pendingWhenUnconfigured: true, notes: 'Optional/pending by design when unconfigured; unauthenticated calls are not retried in a loop to avoid 429s.' },
  ...
  { id: 'nasa', displayName: 'NASA Open APIs', category: 'space_earth', capabilities: ['search'], requiredEnv: ['NASA_API_KEY'], optionalEnv: [], defaultBaseUrls: {}, implemented: false, pendingWhenUnconfigured: false, notes: 'General NASA discovery (NeoWs etc.). Adapter not yet implemented this build phase.' },
  ...
  { id: 'courtlistener', displayName: 'CourtListener', category: 'legal', capabilities: ['search', 'getById'], requiredEnv: ['COURTLISTENER_API_TOKEN'], optionalEnv: [], defaultBaseUrls: {}, implemented: false, pendingWhenUnconfigured: false, notes: 'Adapter not yet implemented this build phase.' },
  { id: 'internet_archive', displayName: 'Internet Archive', category: 'web_archive', capabilities: ['search', 'getById'], requiredEnv: ['INTERNET_ARCHIVE_USER_AGENT_BASE'], optionalEnv: ['INTERNET_ARCHIVE_BASE_URL'], defaultBaseUrls: { INTERNET_ARCHIVE_BASE_URL: 'https://archive.org' }, implemented: false, pendingWhenUnconfigured: false, notes: 'Adapter not yet implemented this build phase.' },
  { id: 'wayback', displayName: 'Wayback Machine', category: 'web_archive', capabilities: ['historicalCaptures', 'compareCaptures'], requiredEnv: [], optionalEnv: ['WAYBACK_BASE_URL'], defaultBaseUrls: { WAYBACK_BASE_URL: 'https://web.archive.org' }, implemented: false, pendingWhenUnconfigured: false, notes: 'Adapter not yet implemented this build phase.' },
  ...
  { id: 'common_crawl', displayName: 'Common Crawl', category: 'web_archive', capabilities: ['search', 'historicalCaptures'], requiredEnv: ['COMMON_CRAWL_USER_AGENT_BASE'], optionalEnv: ['COMMON_CRAWL_INDEX_BASE_URL', 'COMMON_CRAWL_DATA_BASE_URL', 'COMMON_CRAWL_COLLECTIONS_URL'], defaultBaseUrls: { COMMON_CRAWL_INDEX_BASE_URL: 'https://index.commoncrawl.org', COMMON_CRAWL_DATA_BASE_URL: 'https://data.commoncrawl.org', COMMON_CRAWL_COLLECTIONS_URL: 'https://index.commoncrawl.org/collinfo.json' }, implemented: false, pendingWhenUnconfigured: false, notes: 'Adapter not yet implemented this build phase.' },
]
