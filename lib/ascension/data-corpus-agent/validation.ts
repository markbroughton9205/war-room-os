...
export async function runDataCorpusAgentPhase8Validation(): Promise<{
  ...
  results.push(
    check(
      '65_runtime_truth',
      operationalAscensionAgentCount() === 7 &&
  ...
  results.push(
    check(
      '73_exactly_7_operational',
      operationalAscensionAgentCount() === 7 && OPERATIONAL_ASCENSION_AGENTS.length === 7,
      String(operationalAscensionAgentCount()),
    ),
  )
  results.push(
    check(
      ...
      !TARGET_ASCENSION_AGENTS_UNIMPLEMENTED.includes('DATA_CORPUS_AGENT' as never) &&
        ...
        TARGET_ASCENSION_AGENTS_UNIMPLEMENTED.length === 2,
