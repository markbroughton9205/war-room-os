/**
 * #22 Phase 13 — Commander or local-Commander session for WORLD_LEARNING_AGENT.
...
export type WorldLearningAgentCaller =
  | { ok: true; userId: string; surface: 'REMOTE_COMMANDER' | 'LOCAL_COMMANDER' }
  | { ok: false; response: NextResponse }

export async function requireWorldLearningAgentCaller(): Promise<WorldLearningAgentCaller> {
  ...
  const remote = await requireCommanderSession('WORLD_LEARNING_AGENT')
