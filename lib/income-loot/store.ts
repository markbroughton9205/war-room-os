...
export function insertIncomeLootOpportunity(record: Omit<LootOpportunity, 'id' | 'discoveredAt' | 'updatedAt' | 'commanderApprovalRequired'>): LootOpportunity | null { const identity=incomeLootIdentity(record); if (records.some(item => item.ownerUserId===record.ownerUserId && incomeLootIdentity(item).identityKey===identity.identityKey)) return null; const now = new Date().toISOString(); const full: LootOpportunity = { ...record, id: `loot_session_${nextId++}`, discoveredAt: now, updatedAt: now, commanderApprovalRequired: true }; records.unshift(full); if (records.length > MAX_OPPORTUNITIES) records.splice(MAX_OPPORTUNITIES); return full }
