...
export async function runPhase49HValidation():Promise<Phase49HValidation[]>{
  ...
  const verificationUrls:string[]=[];const verified=createPayPalIncomeAdapter({PAYPAL_CLIENT_ID:'live-client',PAYPAL_CLIENT_SECRET:'live-secret',PAYPAL_API_BASE_URL:'https://api-m.paypal.com'},async(input)=>{const url=String(input);verificationUrls.push(url);return url.endsWith('/v1/oauth2/token')?new Response(JSON.stringify({access_token:'token'}),{status:200,headers:{'content-type':'application/json'}}):new Response(JSON.stringify({transaction_details:[]}),{status:200,headers:{'content-type':'application/json'}})},()=>NOW)
  const verifiedStatus=await verified.getConnectionStatus()
  add('phase49h_05b_transaction_search_probe',verifiedStatus.state==='LIVE_VERIFIED'&&verifiedStatus.liveVerified&&verificationUrls.some(url=>url.includes('/v1/reporting/transactions?'))&&!verificationUrls.some(url=>url.includes('/v1/reporting/balances')),'live verification uses the enabled read-only Transaction Search permission and never requires balance access')
