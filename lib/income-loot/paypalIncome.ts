...
export function createPayPalIncomeAdapter(env:Env=process.env,request:typeof fetch=fetch,now=()=>new Date().toISOString()):PayPalIncomeAdapter{
  ...
  const connectionProbePath=(checkedAt:string)=>{const end=new Date(checkedAt),start=new Date(end.getTime()-24*60*60*1000);const params=new URLSearchParams({start_date:start.toISOString(),end_date:end.toISOString(),fields:'transaction_info',page_size:'1',page:'1',balance_affecting_records_only:'Y'});return`/v1/reporting/transactions?${params}`}
  return{
    async getConnectionStatus(){const configured=getPayPalConfiguredStatus(env,now);if(configured.state!=='CONFIGURED_NOT_VERIFIED')return configured;const checkedAt=now();try{const response=await authenticated(connectionProbePath(checkedAt));if(response.ok)return{state:'LIVE_VERIFIED',liveVerified:true,checkedAt,reason:'A real authenticated PayPal Transaction Search request succeeded.'};if(response.status===401||response.status===403)return{state:'ACCESS_DENIED',liveVerified:false,checkedAt,reason:'PayPal denied Transaction Search access.'};return{state:'CONFIGURED_NOT_VERIFIED',liveVerified:false,checkedAt,reason:`PayPal verification returned HTTP ${response.status}.`}}catch(error){const code=error instanceof Error?error.message:'';const state:PayPalConnectionState=PAYPAL_CONNECTION_STATES.includes(code as PayPalConnectionState)?code as PayPalConnectionState:'CONFIGURED_NOT_VERIFIED';return{state,liveVerified:false,checkedAt,reason:'PayPal connectivity was not verified.'}}
    },
