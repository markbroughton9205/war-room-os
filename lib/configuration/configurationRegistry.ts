...
export const PROVIDER_CONFIG_REGISTRY: ProviderConfigDefinition[] = [
  ...
  },
  {
    ...
    alternativeEnvVarGroups: [['MOONSHOT_API_KEY']],
    required: 'recommended',
    powers: ['Kimi family', 'task decomposition', 'execution planning', 'long-context reasoning'],
    affectedFeatures: ['Command Center', 'Stable Group Chat', 'Diagnostics'],
    setupLocation: 'Project or Vercel environment variables',
    lastCheckResult: 'Credential presence only; live probe remains in provider health and engine diagnostics.',
    recommendedNextAction: 'Set KIMI_API_KEY (or MOONSHOT_API_KEY), then verify /api/providers/direct-test?provider=kimi.',
  },
  {
    id: 'tavily',
    name: 'Tavily',
  ...
  },
  {
    ...
    lastCheckResult: 'Persistent source registry and Tavily-backed local queries are wired; operator-maintained feeds remain an optional enrichment.',
    recommendedNextAction: 'No setup action required; optionally curate additional trusted local feeds.',
  },
  {
    id: 'rss_news_sources',
    ...
    category: 'research_intel',
    requiredEnvVars: [],
    alternativeEnvVarGroups: [['GUARDIAN_API_KEY'], ['NEWS_RSS_FEEDS'], ['NEWS_API_KEY']],
    required: 'recommended',
    powers: ['Guardian story cards', 'news cards', 'background source monitoring', 'current local context'],
    affectedFeatures: ['Live Environment', 'Diagnostics'],
    setupLocation: 'Source registry environment variables',
    lastCheckResult: 'Guardian, RSS registry, and NewsAPI key presence only; live news fetch runs server-side in the Live Environment route.',
    missingDependency: 'RSS/news adapter or feed list',
    recommendedNextAction: 'Set GUARDIAN_API_KEY for richer story cards, NEWS_API_KEY for headline cards, or NEWS_RSS_FEEDS with bounded source registry entries.',
  },
  {
    id: 'weather_provider',
    ...
    category: 'research_intel',
    requiredEnvVars: [],
    optionalEnvVars: ['WEATHER_PROVIDER', 'WEATHER_LATITUDE', 'WEATHER_LONGITUDE', 'WEATHER_GOV_CONTACT'],
    alternativeEnvVarGroups: [['WEATHER_API_KEY'], ['WEATHER_GOV_CONTACT'], ['WEATHER_LATITUDE', 'WEATHER_LONGITUDE']],
    required: 'recommended',
    powers: ['weather widget', 'local environment context', 'weather risk alerts'],
    affectedFeatures: ['Live Environment', 'Income Operations'],
    setupLocation: 'Project or Vercel environment variables',
    lastCheckResult: 'Env presence only; live weather fetch runs server-side in the Live Environment route.',
    recommendedNextAction: 'Set WEATHER_API_KEY for OpenWeather/WeatherAPI and WEATHER_GOV_CONTACT for NOAA/NWS fallback. Optional coordinates override the Akron default.',
  },
  {
    ...
    category: 'research_intel',
    requiredEnvVars: [],
    optionalEnvVars: ['ASTROLOGY_PROVIDER'],
    alternativeEnvVarGroups: [['HOROSCOPE_API_KEY']],
    required: 'optional',
  ...
  },
  {
    ...
    optionalEnvVars: ['FINANCE_PROVIDER', 'MARKET_WATCHLIST', 'FINANCE_SYMBOLS'],
    required: 'optional',
    powers: ['market data', 'finance monitoring', 'income opportunity validation'],
    affectedFeatures: ['Income Operations', 'System Health', 'Live Environment'],
    setupLocation: 'Project or Vercel environment variables',
    lastCheckResult: 'Credential presence only; live quotes run server-side in the Live Environment route.',
    recommendedNextAction: 'Set FINANCE_PROVIDER and FINANCE_API_KEY; optionally set MARKET_WATCHLIST for the watchlist.',
  },
  {
    ...
    lastCheckResult: 'Covered by the implemented Research Engine public-data catalog, including government, statistics, legal, transport, weather, and geospatial adapters.',
    recommendedNextAction: 'No setup action required; use Research Engine provider diagnostics for live source checks.',
    staticStatus: 'ready',
  },
  {
    id: 'logistics_data',
    ...
    category: 'research_intel',
    requiredEnvVars: [],
    optionalEnvVars: ['LOGISTICS_API_KEY'],
    required: 'optional',
    powers: ['route status', 'supply chain context', 'transport opportunity checks'],
    affectedFeatures: ['Income Operations', 'Live Environment'],
    setupLocation: 'Project or Vercel environment variables',
    lastCheckResult: 'Optional generic placeholder only; no concrete logistics vendor or adapter has been selected.',
    missingDependency: 'External logistics vendor selection, account, API contract, and credential',
    recommendedNextAction: 'Select a concrete logistics vendor only if commercial logistics coverage is required.',
  ...
  },
  {
    ...
    name: 'Cursor Manual Engineering Layer',
    category: 'operations',
    requiredEnvVars: [],
    optionalEnvVars: ['OPENAI_API_KEY'],
    required: 'future',
    powers: ['Cursor manual task packets', 'cloud provider engineering review prompts'],
    affectedFeatures: ['Agents', 'Diagnostics'],
    setupLocation: 'Cloud provider configuration and manual Cursor workspace',
    lastCheckResult: 'Manual-only; no autonomous external action is wired.',
    recommendedNextAction: 'Use Cursor manually for implementation and keep approvals visible.',
    staticStatus: 'ready',
  },
]
...
export const TAB_CONFIG_REGISTRY: TabConfigDefinition[] = [
  ...
  },
  {
    ...
    providerIds: ['xai', 'tavily', 'firecrawl', 'opportunity_scout', 'finance_market_data', 'government_public_data', 'strategic_memory'],
    description: 'Revenue radar, opportunity scout, finance/logistics/public data inputs, and worker assignment context.',
  },
  {
    id: 'agents',
    name: 'Agents',
    providerIds: ['cursor_codex_future_layer', 'workflow_queue', 'openai', 'anthropic', 'google_gemini'],
    description: 'Cloud agent families, provider readiness, Cursor manual lane, approvals, and Red Team review.',
  },
  {
    id: 'approvals',
    name: 'Approvals',
  ...
  },
  {
    id: 'system',
    name: 'System Health',
    providerIds: ['supabase', 'vercel_runtime', 'event_bus', 'workflow_queue', 'memory_archive', 'red_sentinel'],
    description: 'Operational readiness summary, resources, worker health, deployment awareness, and blockers.',
  },
  {
    id: 'diagnostics',
    name: 'Diagnostics',
    providerIds: ['openai', 'anthropic', 'xai', 'google_gemini', 'tavily', 'firecrawl', 'supabase', 'vercel_runtime', 'red_sentinel', 'cursor_codex_future_layer'],
    description: 'Configuration sweep, engine matrix, runtime integrity, repo awareness, and deployment diagnostics.',
  },
  {
    id: 'live_environment',
    name: 'Live Environment',
    providerIds: ['weather_provider', 'finance_market_data', 'horoscope_provider', 'rss_news_sources', 'local_hyperlocal_sources', 'persistent_source_network', 'tavily', 'firecrawl'],
    description: 'Weather, finance, location, alerts, horoscope, news cards, weak signals, and source-backed context.',
  },
]
