/**
 * PASS 010 live local-14B production mission.
...
async function run() {
  ...
  for (const mission of live) {
    ...
    const zombie = !mission.archived
      && mission.resumeEligible !== false
      && ['FAILED', 'BLOCKED', 'PAUSED', 'RECOVERING', 'WAITING_RESOURCE'].includes(mission.status)
    if (leftover || zombie) {
  ...
  const explained = /selected\.engineeringReviewDetail/.test(panel)
    && /foundry-engineering-review/.test(panel)
  ...
  const homepageOk = /Tell Foundry the result you want/.test(shell)
    ...
    && !/PASS 009/.test(shell)
    && !/PASS 010/.test(shell)
  if (boundedOk && sourceTouched && explained && run.runtimeState.identityMatch === true) {
...
export { run as runFoundryAutonomousEngineeringDepthPass010Proof }
