/**
 * PASS 013 — one harmless locate-only 14B mission. Search+read then COMPLETE. No writes.
 */
...
const REQUEST = 'Where is runProductionLeaseWatchdog defined? Search and read the owner file. Do not change files.'
...
async function run() {
  ...
  const mission = await startMission(REQUEST, 'PASS 013 locate-only')
