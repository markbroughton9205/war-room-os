...
export async function streamGrokChat(input: {
  apiKey: string
  model: string
  system: string
  prompt: string
  ...
  if (!res.ok) {
    let message = `xAI failed (${res.status})`
    try {
  ...
  try {
    ...
    if (text.trim()) {
      return { ok: false, text, partial: true, httpStatus: aborted ? 'timeout' : res.status, error: aborted ? 'stream aborted' : 'grok_stream_error', firstDeltaAt }
    }
    return { ok: false, text: '', partial: false, httpStatus: aborted ? 'timeout' : 'unavailable', error: aborted ? 'stream aborted' : 'grok_stream_error', parserError: !aborted }
  }
