...
export async function runStage3EvalBaselineValidation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check(
    ...
    STAGE3_DESIGN_STATUS === 'ACCEPTED_FOR_PREPARATION'
      ...
      && (NEXT_AUTHORIZED_PASS === 'STAGE3A_COMMANDER_AUTHORIZATION_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3A_COMMANDER_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3A_REVIEW_COMPLETE' || NEXT_AUTHORIZED_PASS === 'STAGE3A_CANDIDATE_SELECTION_COMPLETE' || NEXT_AUTHORIZED_PASS === 'STAGE3A_CANDIDATE_ADJUDICATION_COMPLETE')
