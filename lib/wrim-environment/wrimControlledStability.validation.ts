...
export async function runControlledStabilityValidation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check('42_phase3a_complete', PHASE3A_STATUS === 'PHASE3A_COMPLETE' && (NEXT_AUTHORIZED_PASS === 'PHASE3A_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3_COMMANDER_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3_EXECUTION_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3A_COMMANDER_AUTHORIZATION_REVIEW') && TRAINING_AUTHORIZATION === 'OFF', PHASE3A_STATUS))
