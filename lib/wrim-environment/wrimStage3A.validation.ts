...
export async function runStage3AValidation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check('13_status', (STAGE3A_STATUS === 'COMPLETE_PENDING_REVIEW' || STAGE3A_STATUS === 'REVIEWED') && (STAGE3_TRAINER_STATUS === 'STAGE3A_COMPLETE_PENDING_REVIEW' || STAGE3_TRAINER_STATUS === 'STAGE3A_REVIEWED') && STAGE3A_EXECUTION_READINESS === false && (NEXT_AUTHORIZED_PASS === 'STAGE3A_COMMANDER_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3A_REVIEW_COMPLETE' || NEXT_AUTHORIZED_PASS === 'STAGE3A_CANDIDATE_SELECTION_COMPLETE' || NEXT_AUTHORIZED_PASS === 'STAGE3A_CANDIDATE_ADJUDICATION_COMPLETE'), STAGE3A_STATUS))
