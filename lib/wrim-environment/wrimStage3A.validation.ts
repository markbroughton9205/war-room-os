...
export async function runStage3AValidation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check('5_auth_off', TRAINING_AUTHORIZATION === 'OFF' && STAGE3_AUTHORIZATION === 'NO_PENDING_REVIEW' && runtimePy.includes('TRAINING_AUTHORIZATION = "OFF"') && runtimePy.includes('STAGE3_AUTHORIZATION = "NO_PENDING_REVIEW"'), STAGE3_AUTHORIZATION))
  ...
  results.push(check('13_status', STAGE3A_STATUS === 'COMPLETE_PENDING_REVIEW' && STAGE3_TRAINER_STATUS === 'STAGE3A_COMPLETE_PENDING_REVIEW' && STAGE3A_EXECUTION_READINESS === false && NEXT_AUTHORIZED_PASS === 'STAGE3A_COMMANDER_REVIEW', STAGE3A_STATUS))
