...
export async function runWrimEnvironmentValidation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check(
    ...
    STAGE1_AUTHORIZED === true
      ...
      && (NEXT_AUTHORIZED_PASS === 'PHASE2_GRID_REVIEW' || NEXT_AUTHORIZED_PASS === 'PHASE3A_INTERPOLATION' || NEXT_AUTHORIZED_PASS === 'PHASE3A_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3_COMMANDER_REVIEW'),
    CURRENT_WRIM_TRAINING,
  ))
