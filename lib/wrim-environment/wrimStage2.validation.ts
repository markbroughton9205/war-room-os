...
export async function runWrimStage2Validation(): Promise<{ passed: number; failed: number; results: Check[] }> {
  ...
  results.push(check('50_nothing_deployed', (NEXT_AUTHORIZED_PASS === 'PHASE2_PRIMARY_GRID' || NEXT_AUTHORIZED_PASS === 'PHASE2_GRID_REVIEW' || NEXT_AUTHORIZED_PASS === 'PHASE3A_INTERPOLATION' || NEXT_AUTHORIZED_PASS === 'PHASE3A_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3_COMMANDER_REVIEW' || NEXT_AUTHORIZED_PASS === 'STAGE3_EXECUTION_REVIEW') && FORBIDDEN_ENV_ACTIONS.includes('START_STAGE_3'), 'no deploy; Stage 3 denied'))
