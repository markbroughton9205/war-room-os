...
const repairId = `foundry-commander-ui-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+/, '').toLowerCase()}`
const feature = `foundry-local-model-reliability-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+/, '').toLowerCase()}`
