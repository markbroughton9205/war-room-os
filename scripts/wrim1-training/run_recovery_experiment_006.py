#!/usr/bin/env python3
"""TEST-WRIM1.1-RECOVERY-006. TEST_ONLY. Same interleaved mix as 005; peak LR 3e-5. Does not overwrite 001–005."""
...
EXPERIMENT_ID = "TEST-WRIM1.1-RECOVERY-006"
WORK_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-006"
PRIOR_005_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-005"
...
FULL_DIAG_STEPS = (0, 10, 25, 30, 35, 40, 45, 50)
...
def recovery_config() -> dict:
    ...
    cfg.update({
        ...
        "lr_rationale": (
            "RECOVERY-006: hold Recovery-005 interleaved mix/optimizer/warmup/horizon. "
            "ONLY peak LR changes: 3e-4 -> 3e-5 (10x). Warmup 25 is the repo-consistent "
            "scheduler equivalent (not a second curriculum variable). Cosine floor 10% "
            "(3e-6) with scheduler horizon 150 (train only 50)."
        ),
    })
    return cfg
...
def compare_local_mix_to_005(planned: list[dict], prior_005: Path) -> dict:
    prev = json.loads((prior_005 / "planned-step-source-map.json").read_text(encoding="utf-8"))
    prev_steps = prev.get("steps") or []
    n = min(len(planned), len(prev_steps), MAX_STEPS)
    mismatches = []
    for i in range(n):
        a = planned[i]
        b = prev_steps[i]
        ...
        for key in ("rehearsal_pct", "prose_pct", "code_pct", "json_pct", "behavior_pct"):
            ...
            if abs(da - db) > 1e-6:
                mismatches.append({"step": i + 1, "field": key, "006": da, "005": db})
    pre_a = local_mix_preflight(planned)
    pre_b = prev.get("preflight") or {}
    longest_a = pre_a.get("longest_rehearsal_only_steps")
    longest_b = pre_b.get("longest_rehearsal_only_steps")
    material = bool(mismatches) or longest_a != longest_b
    return {
        "compared_steps": n,
        "seq_and_pct_mismatches": mismatches[:20],
        "n_mismatches": len(mismatches),
        "longest_rehearsal_only_006": longest_a,
        "longest_rehearsal_only_005": longest_b,
...
def main() -> int:
    ...
    priors = [
        ...
        prior_005 / "experiment-summary.json",
        prior_005 / "train.npy",
        prior_005 / "planned-step-source-map.json",
        prior_005 / "retention-windows.npy",
        prior_005 / "WRIM-RECOVERY-005-EXPANDED-100.json",
    ]
    for p in priors:
        if not p.is_file():
            print(f"prior artifact missing ({p}); aborting so 001–005 stay intact", file=sys.stderr)
            return 2
    forbidden = {p.parent.resolve() for p in priors if "TEST-WRIM1.1-RECOVERY-" in str(p)}
    if work.resolve() in forbidden or work.resolve() == prior_005.resolve():
        print("refusing to write 006 into a prior experiment directory", file=sys.stderr)
        return 2
    ...
    prev_train = np.load(prior_005 / "train.npy")
    prev_mask = np.load(prior_005 / "train-mask.npy")
    prev_val = np.load(prior_005 / "val.npy")
    stream_cmp = {
        ...
        "train_len_006": int(train_stream.size),
        "train_len_005": int(prev_train.size),
    }
    (work / "stream-identity-vs-005.json").write_text(json.dumps(stream_cmp, indent=2) + "\n")
    if not (stream_cmp["train_equal"] and stream_cmp["train_mask_equal"]):
        print(json.dumps({
            "error": "packed train stream differs from Recovery-005; comparability broken; not training",
    ...
    if not preflight["passed"]:
        ...
        return 2
    mix_cmp = compare_local_mix_to_005(planned, prior_005)
    (work / "local-mix-vs-005.json").write_text(json.dumps(mix_cmp, indent=2) + "\n")
    if not mix_cmp["passed"]:
        print(json.dumps({
            "error": "local scheduling differs materially from Recovery-005; not training",
            "mix_cmp": mix_cmp,
        }, indent=2))
        return 2
    ...
    run_manifest = {
        ...
        "authorization": "COMMANDER_RECOVERY_006_LOW_LR_INTERLEAVED_ONLY",
    ...
    lr_plan = {
        "peak_lr": PEAK_LR,
        "recovery_005_peak_lr": RECOVERY_005_PEAK_LR,
        "ratio_vs_005": PEAK_LR / RECOVERY_005_PEAK_LR,
        "warmup_steps": WARMUP,
        "warmup_note": "Repo-consistent equivalent of Recovery-005 (25). Not reduced to 10 so step-25/30 LR shape matches 005 at 10x scale.",
    ...
    frozen_windows = np.load(prior_005 / "retention-windows.npy")
    frozen_meta = json.loads((prior_005 / "retention-windows-meta.json").read_text(encoding="utf-8"))
    ret = {**frozen_meta, "windows": frozen_windows, "frozen_from": "TEST-WRIM1.1-RECOVERY-005"}
    ...
    (work / "retention-windows-meta.json").write_text(json.dumps({
        **{k: ret[k] for k in ret if k != "windows"},
        "equal_to_005": bool(np.array_equal(frozen_windows, np.load(prior_005 / "retention-windows.npy"))),
    }, indent=2) + "\n")
    ...
    expanded_suite = json.loads((prior_005 / "WRIM-RECOVERY-005-EXPANDED-100.json").read_text(encoding="utf-8"))
    extra_items = expanded_suite.get("expanded_87") or []
    expanded_suite = {
        **expanded_suite,
        "suite_id": "WRIM-RECOVERY-006-EXPANDED-100",
        "frozen_from": "TEST-WRIM1.1-RECOVERY-005",
        "original_13": suite["items"],
        "n_total": 13 + len(extra_items),
    }
    (work / "WRIM-RECOVERY-006-EXPANDED-100.json").write_text(json.dumps(expanded_suite, indent=2) + "\n")
    ...
    summary = {
        ...
        "parent_load_proof": parent_proof,
        "data_mix": report,
        "stream_identity_vs_005": stream_cmp,
        "local_mix_vs_005": mix_cmp,
        "lr_schedule": {
            "peak_lr": PEAK_LR,
            "initial_lr": initial_lr,
            "warmup_steps": WARMUP,
