#!/usr/bin/env python3
"""TEST-WRIM1.1-RECOVERY-010. TEST_ONLY. Isolates TOOL_USE supervised training vs Recovery-008.

Does not resume Recovery-008. Does not promote. Does not touch production.
Does not start Recovery-011 or WRIM1-RUN-000003.
"""
...
from pack_recovery_010 import (  # noqa: E402
    OFFICIAL_TRAIN_SHA,
    PACK_ID,
    classify,
...
EXPERIMENT_ID = "TEST-WRIM1.1-RECOVERY-010"
WORK_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-010"
    ...
    return {
        ...
        "supervised_ce_aggregate": (
            None if not any(k in acc and acc[k][1] > 0 for k in (
                "INSTRUCTION", "JSON", "WR_CONCEPT", "EVIDENCE", "CORRECTION", "CODE_SUPERVISED", "TOOL"
...
def recovery_config() -> dict:
    ...
    cfg.update({
        ...
        "lr_rationale": (
            "RECOVERY-010: hold Recovery-008 LR exactly. Peak 3e-5, warmup 25, "
            "cosine horizon 150, then hold floor 3e-6 through 250. "
            "Only curriculum change: TOOL_USE supervised windows replaced by WR-CORPUS-0 rehearsal. "
            "Base mix is Recovery-008, not Recovery-009."
        ),
    })
    return cfg
...
def fail_packet(work: Path, reason: str, evidence: dict) -> int:
    payload = {
        ...
        "NOT_OFFICIAL_WRIM_LINEAGE": True,
        "NOT_PRODUCTION": True,
        "verdict": "WRIM-1.1 RECOVERY-010 — FAIL",
        "reason": reason,
        "evidence": evidence,
        "trained": False,
        "promoted": False,
...
def main() -> int:
    ...
    if work.resolve() == official_dir.resolve():
        return fail_packet(work, "refusing to write Recovery-010 into official 000002 directory", {})
    ...
    packed = materialize_recovery_010_pack(root=root, tokenizer=tokenizer)
    report = {
        ...
        "tool_absence_passed": packed["tool_absence_proof"]["passed"],
        ...
        "tool_use_zero": packed["tool_use_zero"],
        ...
        "note": "TEST_ONLY Recovery-010 pack: TOOL_USE supervised windows replaced with WR-CORPUS-0 rehearsal on the Recovery-008 mix; QUALITY_CODE leftover retained.",
    }
    (work / "pack-report.json").write_text(json.dumps({
        **report,
        "token_counts": packed["token_counts"],
        "mask_proof": packed["mask_proof"],
        "tool_absence_proof": packed["tool_absence_proof"],
    ...
    if not packed["non_tool_identity"]["passed"]:
        return fail_packet(work, "non-TOOL_USE windows are not identical to Recovery-008", packed["non_tool_identity"])
    if not packed["tool_use_zero"]["passed"]:
        return fail_packet(work, "TOOL_USE training still present; DO NOT TRAIN", packed["tool_use_zero"])
    if not packed["tool_absence_proof"]["passed"]:
        return fail_packet(work, "tool training residue or held-out TOOL eval identity failed; DO NOT TRAIN", packed["tool_absence_proof"])
    ...
    if packed["token_delta_report"]["rehearsal_010"] >= 300_000:
    ...
    stream_cmp = {
        "010_train_npy_sha256": train_sha,
        ...
        "intentionally_differs_from_008": True,
        "array_equal_008": False,
        "length_equal_008": int(train_stream.size) == EXPECTED_PACK_TOKENS,
        "pack_id": PACK_ID,
    }
    if prior008_npy.is_file():
        off_arr = np.load(prior008_npy)
        stream_cmp["array_equal_008"] = bool(np.array_equal(train_stream, off_arr))
        stream_cmp["008_file_matches_expected"] = sha256_file(prior008_npy) == OFFICIAL_TRAIN_SHA
        off_mask = np.load(prior_008 / "train-mask.npy")
        stream_cmp["mask_array_equal_008"] = bool(np.array_equal(train_mask, off_mask))
        ...
        for m in mapping:
            ...
            if m["changed"]:
                if eq:
                    bad.append({"window": m["window_index"], "reason": "TOOL_USE replacement identical to old tool window"})
                else:
                    diff_ok += 1
            else:
                if not eq:
                    bad.append({"window": m["window_index"], "reason": "non-tool window drifted"})
                else:
                    same_ok += 1
        stream_cmp["non_tool_windows_byte_equal"] = same_ok
        stream_cmp["tool_windows_replaced"] = diff_ok
    ...
    if stream_cmp.get("array_equal_008"):
        return fail_packet(work, "010 stream is identical to 008; TOOL_USE was not replaced", stream_cmp)
    if not stream_cmp.get("length_equal_008"):
        return fail_packet(work, "010 stream length differs from 008 token budget", stream_cmp)
    if prior008_npy.is_file() and not stream_cmp.get("window_byte_proof_passed"):
        return fail_packet(work, "window-level 008→010 identity proof failed", stream_cmp)

    planned = packed["planned_010"]
    preflight = packed["local_mix_preflight"]
    step_map = packed["step_mapping_008_to_010"]
    ...
    (work / "window-mapping-008-to-010.json").write_text(json.dumps({
        "n_windows": len(packed["window_mapping"]),
        "changed_windows": packed["changed_windows"],
        "unchanged_windows": packed["unchanged_windows"],
        "windows": packed["window_mapping"],
    }, indent=2) + "\n")
    (work / "step-mapping-008-to-010.json").write_text(json.dumps({
    ...
    if not step_map["seq_starts_identical"]:
        return fail_packet(work, "seq_starts drifted vs Recovery-008; window lengths not preserved", step_map)
    ...
    (work / "tool-absence-proof.json").write_text(json.dumps(packed["tool_absence_proof"], indent=2) + "\n")
    ...
    if eval_identity["EVAL_TOOL"] != 10 or eval_identity["eval_id"] != EVAL_ID or not eval_identity["matches_pack"]:
        return fail_packet(work, "held-out TOOL eval identity changed", eval_identity)
    ...
    run_manifest = {
        ...
        "authorization": "COMMANDER_TEST_WRIM1_1_RECOVERY_010_ONLY",
        "does_not_overwrite": [
            "WRIM-0", "WRX-000001", "WRIM1-RUN-000001", "WRIM1-RUN-000002",
            "Recovery-001-009", "WR-CORPUS-1.1-CAPABILITY-CANDIDATE", "WR-CORPUS-1.1-RECOVERY-009-NO-CODE",
        ],
        "does_not_start": ["WRIM1-RUN-000003", "Recovery-011", "promotion", "production", "WRIM-1.2"],
    }
    ...
    def class_mix_for_starts(starts: list[int]) -> dict:
        ...
        return {
            ...
            "tool_tokens": int(by.get("TOOL") or 0),
            "tool_pct": 100.0 * int(by.get("TOOL") or 0) / tot,
        }
    ...
    try:
        while global_step < MAX_STEPS:
            ...
            if cmix["tool_tokens"] > 0:
                tool_use_batches += 1
                early_stop = {"stopped": True, "reason": "TOOL_USE tokens appeared in a training batch", "step": global_step + 1}
                break
            if cmix["dominant_class"] == "TOOL" or float(cmix["tool_pct"]) >= 15:
    ...
    pass_ok = bool(
        ...
        and better_100_120
        and tool_use_batches == 0
    )
    verdict = "WRIM-1.1 RECOVERY-010 — PASS" if pass_ok else "WRIM-1.1 RECOVERY-010 — FAIL"
    ...
    compare_008 = {
        str(s): {
            "010": brief_diag(next((r for r in diag_table if r.get("step") == s), None)),
    ...
    family_grad_summary["TOOL"] = family_grad_summary.get("TOOL") or {
        "n": 0, "mean": None, "median": None, "max": None, "expected_zero_batches": True,
    }
    ...
    summary = {
        ...
        "step_mapping_008_to_010": {k: packed["step_mapping_008_to_010"][k] for k in packed["step_mapping_008_to_010"] if k != "steps"},
        "mask_proof": packed["mask_proof"],
        "tool_absence_proof": packed["tool_absence_proof"],
