#!/usr/bin/env python3
"""TEST-WRIM1.1-RECOVERY-009. TEST_ONLY. Isolates QUALITY_CODE vs Recovery-008.

Does not resume Recovery-008. Does not promote. Does not touch production.
Does not start Recovery-010 or WRIM1-RUN-000003.
"""
...
from pack_recovery_009 import (  # noqa: E402
    OFFICIAL_TRAIN_SHA,
    classify,
    materialize_recovery_009_pack,
    objective_of,
)
...
EXPERIMENT_ID = "TEST-WRIM1.1-RECOVERY-009"
WORK_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-009"
...
def recovery_config() -> dict:
    ...
    cfg.update({
        ...
        "lr_rationale": (
            "RECOVERY-009: hold Recovery-008 LR exactly. Peak 3e-5, warmup 25, "
            "cosine horizon 150, then hold floor 3e-6 through 250. "
            "Only curriculum change: QUALITY_CODE leftover replaced by WR-CORPUS-0 rehearsal."
        ),
    })
    return cfg
...
def fail_packet(work: Path, reason: str, evidence: dict) -> int:
    payload = {
        ...
        "NOT_OFFICIAL_WRIM_LINEAGE": True,
        "NOT_PRODUCTION": True,
        "verdict": "WRIM-1.1 RECOVERY-009 — FAIL",
        "reason": reason,
        "evidence": evidence,
        "trained": False,
        "promoted": False,
...
def main() -> int:
    ...
    if work.resolve() == official_dir.resolve():
        return fail_packet(work, "refusing to write Recovery-009 into official 000002 directory", {})
    ...
    packed = materialize_recovery_009_pack(root=root, tokenizer=tokenizer)
    report = {
        ...
        "quality_code_zero": packed["quality_code_zero"],
        "code_supervised_retained": packed["code_supervised_retained"],
        "token_delta_report": packed["token_delta_report"],
        "composition_008": packed["composition_008"],
        "composition_009": packed["composition_009"],
        "non_code_identity": packed["non_code_identity"],
        "note": "TEST_ONLY Recovery-009 pack: QUALITY_CODE leftover windows replaced with WR-CORPUS-0 rehearsal; all other windows byte-identical to Recovery-008.",
    }
    ...
    if not packed["non_code_identity"]["passed"]:
        return fail_packet(work, "non-QUALITY_CODE windows are not identical to Recovery-008", packed["non_code_identity"])
    if not packed["quality_code_zero"]["passed"]:
        return fail_packet(work, "QUALITY_CODE leftover still present", packed["quality_code_zero"])
    if not packed["code_supervised_retained"]["passed"]:
        return fail_packet(work, "supervised CODE family not retained identically", packed["code_supervised_retained"])
    ...
    stream_cmp = {
        "009_train_npy_sha256": train_sha,
    ...
    if prior008_npy.is_file():
        ...
        # non-code positions should match; code positions should differ
        ...
        for m in mapping:
            ...
            if m["changed"]:
                if eq:
                    bad.append({"window": m["window_index"], "reason": "QUALITY_CODE replacement identical to old code"})
                else:
                    diff_ok += 1
            else:
                if not eq:
                    bad.append({"window": m["window_index"], "reason": "non-code window drifted"})
                else:
                    same_ok += 1
        stream_cmp["non_code_windows_byte_equal"] = same_ok
        stream_cmp["code_windows_replaced"] = diff_ok
    ...
    if stream_cmp.get("array_equal_008"):
        return fail_packet(work, "009 stream is identical to 008; QUALITY_CODE was not replaced", stream_cmp)
    if not stream_cmp.get("length_equal_008"):
        return fail_packet(work, "009 stream length differs from 008 token budget", stream_cmp)
    if prior008_npy.is_file() and not stream_cmp.get("window_byte_proof_passed"):
        return fail_packet(work, "window-level 008→009 identity proof failed", stream_cmp)

    planned = packed["planned_009"]
    preflight = packed["local_mix_preflight"]
    step_map = packed["step_mapping_008_to_009"]
    ...
    (work / "window-mapping-008-to-009.json").write_text(json.dumps({
        "n_windows": len(packed["window_mapping"]),
        "changed_windows": packed["changed_windows"],
        "unchanged_windows": packed["unchanged_windows"],
        "windows": packed["window_mapping"],
    }, indent=2) + "\n")
    (work / "step-mapping-008-to-009.json").write_text(json.dumps({
    ...
    if step_map["steps_with_quality_code_in_009"] != 0:
        return fail_packet(work, "planned 009 steps still contain QUALITY_CODE leftover", step_map)
    ...
    run_manifest = {
        ...
        "authorization": "COMMANDER_TEST_WRIM1_1_RECOVERY_009_ONLY",
        "does_not_overwrite": [
            "WRIM-0", "WRX-000001", "WRIM1-RUN-000001", "WRIM1-RUN-000002",
            "Recovery-001-008", "WR-CORPUS-1.1-CAPABILITY-CANDIDATE",
        ],
        "does_not_start": ["WRIM1-RUN-000003", "Recovery-010", "promotion", "production", "WRIM-1.2"],
    }
    ...
    pass_ok = bool(
        ...
        and better_100_120
        and quality_code_batches == 0
    )
    verdict = "WRIM-1.1 RECOVERY-009 — PASS" if pass_ok else "WRIM-1.1 RECOVERY-009 — FAIL"
    ...
    compare_008 = {
        str(s): {
            "009": brief_diag(next((r for r in diag_table if r.get("step") == s), None)),
            "008": brief_diag(load_prior_diag(prior_008, s)),
        }
        for s in (75, 100, 120, 125, 150)
    }
    ...
    family_grad_summary["QUALITY_CODE"] = family_grad_summary.get("QUALITY_CODE") or {
    ...
    summary = {
        ...
        "step_mapping_008_to_009": {k: packed["step_mapping_008_to_009"][k] for k in packed["step_mapping_008_to_009"] if k != "steps"},
        "mask_proof": packed["mask_proof"],
        "tool_proof": packed["tool_proof"],
