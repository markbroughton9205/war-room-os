#!/usr/bin/env python3
"""TEST-WRIM1.1-RECOVERY-002. TEST_ONLY. Does not overwrite RECOVERY-001."""
...
EXPERIMENT_ID = "TEST-WRIM1.1-RECOVERY-002"
WORK_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-002"
...
def recovery_config() -> dict:
    ...
    cfg.update({
        ...
        "lr_rationale": (
            "RECOVERY-002: continued pretrain from WRIM-0 at 1e-4 peak (3x below RECOVERY-001 3e-4; "
            "30x below Genesis 3e-3). Warmup 10 of 50, cosine to 10% floor (1e-5)."
        ),
    })
    return cfg
...
def collapse_gate_002(step0: dict, now: dict, logits: dict) -> tuple[bool, str]:
...
def main() -> int:
    ...
    if not (prior_root / "experiment-summary.json").is_file():
        ...
        return 2
    if work.resolve() == prior_root.resolve():
        print("refusing to write 002 into 001 directory", file=sys.stderr)
        return 2
    ...
    if code_pct > 23.0:
        print(json.dumps({"error": "code share exceeded isolation cap", "code_pct": code_pct}))
        return 2
    ...
    run_manifest = {
        ...
        "authorization": "COMMANDER_LOW_LR_CAPPED_REHEARSAL_ONLY",
        "prior_experiment_id": "TEST-WRIM1.1-RECOVERY-001",
    }
    ...
    while global_step < MAX_STEPS:
        ...
        if global_step in DIAG_STEPS:
            ...
            stop, reason = collapse_gate_002(step0_row, {**suite_out, **symbol_run(suite_out["sky_continuation"])}, lg)
            if nan_inf or stop:
    ...
    summary = {
        ...
        "prior_dir_untouched": str(work.resolve()) != str(prior_root.resolve()),
