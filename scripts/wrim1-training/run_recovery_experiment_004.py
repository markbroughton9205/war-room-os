#!/usr/bin/env python3
"""TEST-WRIM1.1-RECOVERY-004. TEST_ONLY. 30% rehearsal control. Does not overwrite 001/002/003."""
...
EXPERIMENT_ID = "TEST-WRIM1.1-RECOVERY-004"
WORK_REL = "model-lab/manifests/wrim1_1_recovery/test-only/TEST-WRIM1.1-RECOVERY-004"
...
def recovery_config() -> dict:
    ...
    cfg.update({
        ...
        "lr_rationale": (
            "RECOVERY-004: isolate rehearsal at 30%. Peak 3e-4, warmup 25, cosine floor 10% "
            "with Recovery-001 scheduler horizon of 150 steps (train only 50). Do not change optimizer."
        ),
    })
    return cfg
...
def collapse_gate_004(step0: dict, now: dict, logits: dict, grad_row: dict | None = None) -> tuple[bool, str]:
    ...
    if grad_row and not grad_row.get("finite", True):
        return True, "non-finite gradients"
    if grad_row and step0.get("global_grad_l2"):
        g0 = float(step0["global_grad_l2"])
        g = float(grad_row.get("global_grad_l2") or 0)
        if g0 > 0 and g > 50.0 * g0:
            return True, "gradient L2 pathological vs this-run baseline"
    return False, ""
...
def main() -> int:
    ...
    prior_003 = prior_root / "TEST-WRIM1.1-RECOVERY-003"
    ...
    if not (prior_002 / "experiment-summary.json").is_file():
        ...
        return 2
    if not (prior_003 / "experiment-summary.json").is_file():
        print("RECOVERY-003 summary missing; aborting", file=sys.stderr)
        return 2
    if work.resolve() in (prior_root.resolve(), prior_002.resolve(), prior_003.resolve()):
        print("refusing to write 004 into 001/002/003 directory", file=sys.stderr)
        return 2
    ...
    if not audit["passed"]:
        print(json.dumps({"error": "causal-target audit failed; not training", "audit": {k: audit[k] for k in audit if k != "examples"}}))
        return 2
    ...
    if not unit_masks["passed"]:
        print(json.dumps({"error": "unit-level mask audit failed; not training", "unit_masks": unit_masks}))
        return 2
    ...
    run_manifest = {
        ...
        "authorization": "COMMANDER_RECOVERY_004_30PCT_REHEARSAL_CONTROL_ONLY",
    ...
    expanded_suite = {
        "suite_id": "WRIM-RECOVERY-004-EXPANDED-100",
    ...
    (work / "WRIM-RECOVERY-004-EXPANDED-100.json").write_text(json.dumps(expanded_suite, indent=2) + "\n")
    ...
    while global_step < MAX_STEPS:
        ...
        last_grad_row = ginfo
        grad_rows.append({"step": global_step + 1, **ginfo, "learning_rate": lr})
        ...
        val_metrics = None
        if global_step % int(cfg["validation_cadence_steps"]) == 0 or global_step in DIAG_STEPS or global_step in LIGHT_STEPS:
        ...
        if global_step in DIAG_STEPS:
            ...
            suite_out, lg, row = diagnose(global_step, val_metrics, loss_val, full=True)
            ...
            if nan_inf or stop:
                ...
                break
        elif global_step in LIGHT_STEPS:
            suite_out, lg, row = diagnose(global_step, val_metrics, loss_val, full=False)
            stop, reason = collapse_gate_004(
                step0_row,
                {**suite_out, **symbol_run(suite_out["sky_continuation"]), "global_grad_l2": ginfo.get("global_grad_l2")},
                lg,
    ...
    summary = {
        ...
        "prior_003_dir_untouched": str(work.resolve()) != str(prior_003.resolve()),
