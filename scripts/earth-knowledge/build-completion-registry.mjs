...
const PROVIDER_MATCHERS = {
  ...
  // --- Built during the 2026-08-28 source completion pass (Lane G) ---
  bank_of_england_iadb: ['Bank of England Statistical Interactive DB'],
}
...
const IMPLEMENTED_PROVIDERS = new Set([
  ...
  // Transportation providers built by a parallel lane (2026-08-28); no Earth Knowledge
  // Registry rows match these War Room-specific traffic sources, but the set is kept
  // honest about the full implemented provider list.
  'hong_kong_td_cameras', 'quebec_511_cameras', 'quebec_511_events',
  'jartic_traffic_volumes', 'wzdx_wsdot', 'wzdx_iowa_dot', 'wzdx_kytc',
])
...
const LIVE_VERIFIED_PROVIDERS = new Set([
  ...
  'scb_sweden', 'ssb_norway', 'statfin_finland', 'statistics_denmark', 'un_desa_population',
  // Confirmed live during the 2026-08-28 source completion pass (Lane G): real CSV
  // response with real Jan-2024 Bank Rate (5.25) and SONIA observations via direct curl.
  'bank_of_england_iadb',
])
...
// Mission vocabulary (Global Research Engine completion pass, 2026-08-28):
// LIVE / PARTIAL / REGISTERED / MISSING / DUPLICATE / ACCESS REQUIRED / LICENSING REVIEW
const MISSION_VOCAB = {
  LIVE: ['LIVE_IMPLEMENTED'],
  PARTIAL: ['IMPLEMENTED_NOT_LIVE_VERIFIED', 'IMPLEMENTED_ACCESS_DEGRADED'],
  REGISTERED: ['STUB_ONLY'],
  'ACCESS REQUIRED': ['IMPLEMENTED_CREDENTIAL_BLOCKED'],
  'LICENSING REVIEW': ['COMMERCIAL_GATED'],
  DUPLICATE: ['DUPLICATE_COVERAGE'],
  MISSING: ['MISSING', 'SEARCH_INTERFACE_ONLY', 'BULK_ONLY', 'DISCONTINUED', 'EXTERNAL_BLOCKER'],
}
md += `\n## Mission vocabulary roll-up\n\n| Mission status | Count | Fine-grained states included |\n|---|---|---|\n`
for (const [label, states] of Object.entries(MISSION_VOCAB)) {
  const total = states.reduce((sum, s) => sum + (counts[s] ?? 0), 0)
  md += `| ${label} | ${total} | ${states.join(', ')} |\n`
}
