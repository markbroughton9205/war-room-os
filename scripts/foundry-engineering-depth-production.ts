...
const repairId = process.env.FOUNDRY_PRODUCTION_MISSION_ID?.trim() || `foundry-pass009-${stamp}`
const feature = process.env.FOUNDRY_PRODUCTION_FEATURE?.trim() || `foundry-production-ownership-a11y-archive-${stamp}`
...
console.log(`MISSION ${repairId} feature=${feature} skipBuild=${skipBuild}`)
