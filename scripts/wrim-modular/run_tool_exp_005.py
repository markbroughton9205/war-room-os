#!/usr/bin/env python3
"""WR-TOOL PARAMETER-ISOLATED EXPERIMENT 005 — authorized training.

Frozen WRIM-0 + fresh LoRA r=2 on attn.q/v + Linear(256→6) classifier.
Train: frozen V5 train.jsonl. Eval: EVAL-5 val/test. EVAL-4 is secondary historical only.
...
from exp005_support import (  # noqa: E402
    HARD_BOUNDARY_PAIRS_V5 as HARD_BOUNDARY_PAIRS,
    assert_eval5_contract,
    class_weights_from_train,
    eval4_still_frozen,
    eval5_hashes,
    is_hard_boundary_row_v5 as is_hard_boundary_row,
    load_eval5_split,
    load_v5_train,
    v5_gates,
    v5_hashes,
...
OPTIMIZER_RATIONALE = (
    "EXP-005: AdamW lr=5e-4, betas=(0.9,0.999), eps=1e-8, wd=0.01, batch=4, "
    "clip_norm=1.0, max 18 / min 4 / patience 5 on val macro F1. "
    "Class weights w_c = N/(K*n_c) computed from frozen V5 train before optimizer.step. "
    "LoRA r=2 q+v all 18 layers; Linear(256→6); WRIM-0 frozen."
)
...
def fail(work: Path, reason: str, extra: dict | None = None) -> int:
    payload = {"reason": reason, "status": "FAIL", "experiment_id": EXP005_ID, "run_id": EXP005_RUN_ID}
    if extra:
...
def main() -> int:
    ...
    lora_dir = work / "module" / EXP005_LORA_ID
    head_dir = work / "module" / EXP005_HEAD_ID
    ...
    train_hash = hashlib.sha256((V5_CANDIDATE_DIR / "train.jsonl").read_bytes()).hexdigest()
    eval5_h = eval5_hashes()
    eval5_bundle = eval5_h["combined_bundle"]
    eval4_bundle = json.loads((TOOL_EVAL_4_DIR / "HASHES.json").read_text())["combined_bundle"]
    if not eval4_still_frozen():
        return fail(work, "EVAL-4 hash drifted")
    gates_fixed = v5_gates()
    global GATE_ACC, GATE_BAL, GATE_F1, GATE_RECALL, GATE_BOUNDARY, GATE_REAL, KEYWORD_TEST_ACC, BOW_TEST_ACC
    GATE_ACC = float(gates_fixed["primary_accuracy"])
    GATE_BAL = float(gates_fixed["balanced_accuracy"])
    GATE_F1 = float(gates_fixed["macro_f1"])
    GATE_RECALL = float(gates_fixed["per_class_recall_floor"])
    GATE_BOUNDARY = float(gates_fixed["hard_boundary_accuracy"])
    GATE_REAL = float(gates_fixed["REAL_TEST_minimum"])
    KEYWORD_TEST_ACC = float(gates_fixed["strongest_simple_baseline_test_accuracy"])
    BOW_TEST_ACC = float(gates_fixed["strongest_simple_baseline_test_accuracy"])
    class_w = class_weights_from_train(train)
    write_json(work / "class-weights.json", {
        "formula": "w_c = N / (K * n_c)",
        "N": len(train),
        "K": 6,
        "weights": class_w,
        "fixed_before_optimizer": True,
    })
    ...
    write_json(work / "train-class-distribution.json", {
        "n": len(train),
        "counts": class_counts,
        "class_weighting": "w_c = N / (K * n_c) fixed from frozen V5 train before optimizer",
        "reason": "Deterministic inverse-frequency weights from the frozen training distribution.",
    ...
    if leak_val or leak_test or overlap_ids:
        return fail(work, "EVAL-5 overlap with train", {"leak_val": leak_val, "leak_test": leak_test, "ids": overlap_ids})
    ...
    pre_proofs = {
        ...
        "3_train_hash_exact": train_hash == v5_hashes()["train.jsonl"],
        "4_eval5_hash_exact": eval5_bundle == eval5_h["combined_bundle"],
        "4b_eval4_held_out_frozen": eval4_bundle == EVAL4_BUNDLE,
        "5_class_map_exact": list(CLASS_NAMES) == ["NO_TOOL", "WEB", "MEMORY", "FILES", "RESEARCH", "SHA256"],
        "10_eval5_excluded_from_train": not leak_val and not leak_test and not overlap_ids,
    ...
    for key in ("1_wrim0_sha_exact", "2_core_trainable_params_0", "3_train_hash_exact", "4_eval5_hash_exact",
                "4b_eval4_held_out_frozen", "5_class_map_exact", "10_eval5_excluded_from_train", "12_no_production_path"):
    ...
    head_manifest = make_tool_head_manifest(
        module_id=EXP005_HEAD_ID,
        n_classes=N_CLASSES,
        state="SHADOW",
        training_dataset_identity="WR-TOOL-CURRICULUM-V5-CANDIDATE/train.jsonl",
        eval_identity="WR-TOOL-EVAL-5-CANDIDATE",
        experiment_id=EXP005_ID,
        kind="WR_TOOL_PARAMETER_ISOLATED_EXPERIMENT_005",
        extra_provenance={"paired_lora_module": EXP005_LORA_ID, "class_map": list(CLASS_NAMES), "not_merged_checkpoint": True},
    )
    ...
    print(
        f"EXP005 runtime estimate likely={timing['estimated_likely_sec']:.1f}s "
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for chunk in batches(n_tr, BATCH, rng):
            ...
            if any(eid in eval_ids for eid in batch_ids):
                return fail(work, "EVAL-5 entered training batch", {"epoch": epoch, "ids": batch_ids})
            ...
            def step_loss():
                ...
                for j in local:
                    ...
                    _, pred = runtime(idx)
                    ce = nn.losses.cross_entropy(pred, yb)
                    w = mx.array([class_w[CLASS_NAMES[int(y_tr[j])]]], dtype=mx.float32)
                    total = total + mx.mean(ce * w)
    ...
    real_t = subset_accuracy(test, te_pred, lambda r: r.get("source_type") in ("REAL_TEST", "REAL_RUNTIME"))
    fixture_t = subset_accuracy(test, te_pred, lambda r: r.get("source_type") == "TEST_FIXTURE")
    synth_t = subset_accuracy(test, te_pred, lambda r: r.get("source_type") in ("EVAL_SYNTHETIC", "SYNTHETIC"))
    ...
    e4_test_rows = load_eval4_split("test")
    e4_tok = [encode_prefix(tokenizer, r["prompt_prefix"]) for r in e4_test_rows]
    hist_eval4_rep = eval_split(runtime, e4_tok, labels(e4_test_rows))
    hist_eval4 = {
        "n": hist_eval4_rep["n"],
        "accuracy": hist_eval4_rep["accuracy"],
        "balanced_accuracy": hist_eval4_rep["balanced_accuracy"],
        "macro_f1": hist_eval4_rep["macro_f1"],
        "per_class_recall": {c: hist_eval4_rep["per_class"][c]["recall"] for c in CLASS_NAMES},
        "secondary_diagnostic_only": True,
        "not_used_for_checkpoint_or_gates": True,
    }
    ...
    overfit = bool(final_train_acc >= 0.96 and gap >= 0.20)

    if isolation_ok and all_gates and not overfit:
        cap_label = "WR-TOOL EXP005 — CAPABILITY ACQUISITION DEMONSTRATED"
        experiment_verdict = "WR-TOOL EXPERIMENT 005 TRAINING — PASS"
    elif isolation_ok:
        cap_label = "WR-TOOL EXP005 — CAPABILITY ACQUISITION NOT DEMONSTRATED"
        experiment_verdict = "WR-TOOL EXPERIMENT 005 TRAINING — PASS"
        if overfit:
            cap_label = "WR-TOOL EXP005 — CAPABILITY ACQUISITION NOT DEMONSTRATED"
    else:
        cap_label = "WR-TOOL EXP005 — EXPERIMENT EXECUTION FAIL"
        experiment_verdict = "WR-TOOL EXPERIMENT 005 TRAINING — FAIL"
    ...
    write_json(ckpt_dir / "dataset-hashes.json", {
        "train": train_hash,
        "eval5_bundle": eval5_bundle,
        "eval4_bundle_historical": eval4_bundle,
    ...
    composed = attach_module_to_runtime(local_rt, EXP005_LORA_ID)
    composed = attach_module_to_runtime(composed, EXP005_HEAD_ID)
    detached_rt = detach_module_from_runtime(composed, EXP005_HEAD_ID)
    detached_rt = detach_module_from_runtime(detached_rt, EXP005_LORA_ID)
    ...
    write_json(work / "config.json", {
        ...
        "loss": "class_weighted_cross_entropy_w_c=N/(K*n_c)",
    ...
    write_json(work / "dataset-bindings.json", {
        "train": {"hash": train_hash, "n": len(train), "path": str(V5_CANDIDATE_DIR / "train.jsonl")},
        "eval5": {"hash": eval5_bundle, "val_n": len(val), "test_n": len(test), "path": str(TOOL_EVAL_5_DIR)},
        "eval4_historical": {"hash": eval4_bundle, "path": str(TOOL_EVAL_4_DIR), "not_used_for_selection": True},
        "eval5_must_not_enter_optimizer": True,
    ...
    write_json(work / "fixture-metrics.json", fixture_t)
    ...
    write_json(work / "memory-interpretation.json", {
        ...
        "live_store": "V5 uses labeled DEVELOPMENT TEST MEMORY fixtures (TEST_FIXTURE), not production memory",
        "held_out_MEMORY": "family-isolated EVAL-5 MEMORY, largely TEST_FIXTURE",
    ...
    write_json(work / "experiment-runtime.json", {
        ...
        "composed_candidate": f"{WRIM0_ID}+{EXP005_LORA_ID}+{EXP005_HEAD_ID}",
