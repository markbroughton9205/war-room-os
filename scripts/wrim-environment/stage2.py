"""Stage 2 WRIM-0 stability run: exactly 50 AdamW steps maximum, then STOP.

WRIM1-NEBULA-STAB-000001. TEST_ONLY. Not promotion. Not Stage 3.
Starts from WRIM-0 parent weights (not Stage 1, not collapsed WRIM-1).
Fresh AdamW. Ratio-controlled contiguous packing. Collapse/retention sentinels.
"""
...
from stage2_pack import (
    ...
    REHEARSAL_TOLERANCE_PP,
    STEPS,
    build_stage2_streams,
)
...
EVAL_EVERY = 10
STARTING_REASON = (
    "Fresh Stage 2 from WRIM-0 parent weights. Stage 1 (WRIM1-NEBULA-DIAG-000001) "
    "remains historical TEST_ONLY plumbing and is not inherited. Collapsed/rejected "
    "WRIM-1 checkpoints are not parents. Fresh AdamW; no MLX opt.* resume."
)
...
def fail_payload(extra: dict) -> dict:
    return {
        ...
        "WRIM_STAGE2": extra.get("WRIM_STAGE2", "STAGE2_FAIL"),
...
def main() -> int:
    ...
    packed = build_stage2_streams(dump_root=dump_root, tokenizer=tokenizer)
    ...
    if not audit["packing_ok"]:
        payload = fail_payload({"error": "PACKING_AUDIT_FAILED", "packing": audit, "WRIM_STAGE2": "STAGE2_FAIL"})
        write_json(report_path, payload)
    ...
    parent_hash = tensors_sha256({k: v.detach().cpu() for k, v in model.state_dict().items()})
    ...
    step0["step"] = 0
    evals.append(step0)
    print(json.dumps({"eval_step": 0, "collapsed": step0["collapsed_probes"], "retention": step0["retention_family"], "val": step0["validation_loss"], "uniq": step0["mean_unique_ratio"]}), flush=True)
    ...
    for step in range(STEPS):
        ...
        if not bool(torch.isfinite(loss).item()):
            verdict = "STAGE2_STOPPED_BY_SENTINEL"
        ...
        if completed_steps % EVAL_EVERY == 0:
            ...
            evals.append(ev)
            print(json.dumps({"eval_step": completed_steps, "collapsed": ev["collapsed_probes"], "retention": ev["retention_family"], "val": ev["validation_loss"], "uniq": ev["mean_unique_ratio"]}), flush=True)
    ...
    if verdict == "STAGE2_PASS" and not pass_ok:
        verdict = "STAGE2_FAIL"
        stop_reason = stop_reason or "post-run gate failed"

    ready_s3 = "YES" if verdict == "STAGE2_PASS" else "NO"
    next_pass = "READY_FOR_STAGE3_TRAINING_AUTHORIZATION" if verdict == "STAGE2_PASS" else "STAGE2_FAILED_OR_STOPPED"

    lr_schedule = [lr_at_step(i) for i in range(min(completed_steps, STEPS) or 1)]
    manifest = {
        "run_id": RUN_ID,
        "classification": "TEST_ONLY / STABILITY / NOT_PROMOTION_CANDIDATE",
        ...
        "lr_schedule": "warmup 25 then cosine over remaining 25; peak 3e-5 min 3e-6",
    ...
    payload = {
        ...
        "TRAINING_AUTHORIZATION": "OFF",
        "TRAINING_AUTHORIZATION_DURING_RUN": "STAGE2_ONLY",
        "CURRENT_PRODUCTION_WRIM": "NOT_IMPLEMENTED",
        "QWEN": "THIRD_PARTY_MODEL_RUNNING_LOCALLY",
        "RAEL": "NOT_IMPLEMENTED",
