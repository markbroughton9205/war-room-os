...
export function MatrixBackground({
  ...
  contained?: boolean
  channelOverride?: 'cyan' | 'violet' | 'amber' | 'green' | 'red' | 'white' | null
  ...
  return (
    <div
      className={contained
        ? 'pointer-events-none absolute inset-0 z-0 overflow-hidden bg-[#020403]'
        : 'pointer-events-none fixed inset-0 z-0 overflow-hidden bg-[#020617]'}
      aria-hidden
      data-testid={contained ? 'foundry-matrix-background' : undefined}
    >
      {!contained ? (
        <div
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_0%,rgba(0,40,18,0.35),transparent_55%)]" />
      )}
      {!contained ? (
        <div
      ...
      {!reduceMotion ? <MatrixCodeRain contained={contained} channelOverride={channelOverride} intensity={intensity} /> : null}
      <div
      ...
      <div
        ...
        style={{
          background: contained
            ? 'linear-gradient(to bottom, rgba(0,0,0,0.12), transparent 18%, transparent 82%, rgba(0,0,0,0.28))'
            : `
