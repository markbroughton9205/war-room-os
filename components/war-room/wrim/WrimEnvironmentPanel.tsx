  ...
  return (
    <section className="space-y-3 rounded border border-cyan-800 bg-zinc-950 p-4 text-sm text-cyan-100">
      ...
      <article className="grid gap-1 text-xs">
        ...
        <p>STAGE3A status: {String((status as { stage3a_status?: string } | null)?.stage3a_status ?? 'COMPLETE_PENDING_REVIEW')} · classification: {String((status as { stage3a_classification?: string } | null)?.stage3a_classification ?? 'B. REVIEW_REQUIRED_CONTINUOUS_DRIFT')} · candidate: {String((status as { stage3a_candidate_state?: string } | null)?.stage3a_candidate_state ?? 'EVALUATION_CANDIDATE')}</p>
        ...
        <p>next pass: {status?.next_authorized_pass ?? 'STAGE3A_COMMANDER_REVIEW'} (STAGE3A complete pending review; STAGE3B not authorized; training OFF; no Train button)</p>
      </article>
