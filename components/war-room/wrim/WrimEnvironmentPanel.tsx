  ...
  return (
    <section className="space-y-3 rounded border border-cyan-800 bg-zinc-950 p-4 text-sm text-cyan-100">
      ...
      <article className="grid gap-1 text-xs">
        ...
        <p>execution review: {String((status as { stage3_execution_review?: string } | null)?.stage3_execution_review ?? 'PASS')} · trainer: {String((status as { stage3_trainer_status?: string } | null)?.stage3_trainer_status ?? 'STAGE3A_COMPLETE_PENDING_REVIEW')}</p>
        <p>STAGE3A status: {String((status as { stage3a_status?: string } | null)?.stage3a_status ?? 'REVIEWED')} · review: {String((status as { stage3a_review_status?: string } | null)?.stage3a_review_status ?? 'COMPLETE')} · selection: {String((status as { stage3a_candidate_selection_status?: string } | null)?.stage3a_candidate_selection_status ?? 'COMPLETE')} · adjudication: {String((status as { stage3a_candidate_adjudication_status?: string } | null)?.stage3a_candidate_adjudication_status ?? 'COMPLETE')} · classification: {String((status as { stage3a_classification?: string } | null)?.stage3a_classification ?? 'B. REVIEW_REQUIRED_CONTINUOUS_DRIFT')} · candidate: {String((status as { stage3a_candidate_state?: string } | null)?.stage3a_candidate_state ?? 'EVALUATION_CANDIDATE')}</p>
        <p>preferred evaluation candidate: {String((status as { preferred_stage3a_evaluation_candidate?: string } | null)?.preferred_stage3a_evaluation_candidate ?? 'NO_CLEAR_WINNER')} · kind: {String((status as { stage3a_preferred_kind?: string } | null)?.stage3a_preferred_kind ?? 'UNRESOLVED_PARETO')} · not promoted</p>
        <p>STAGE3A readiness: {String((status as { stage3a_execution_readiness?: boolean } | null)?.stage3a_execution_readiness ?? false)} · STAGE3B readiness: {String((status as { stage3b_execution_readiness?: boolean } | null)?.stage3b_execution_readiness ?? false)} · healthy for continuation: {String((status as { stage3a_healthy_for_continuation?: boolean } | null)?.stage3a_healthy_for_continuation ?? false)}</p>
        <p>next pass: {status?.next_authorized_pass ?? 'STAGE3A_CANDIDATE_ADJUDICATION_COMPLETE'} (STAGE3A candidate adjudication complete; no clear interpolant winner; STAGE3B not authorized; training OFF; no Train button)</p>
      </article>
