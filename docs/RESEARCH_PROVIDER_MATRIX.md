...
| GitHub | code | `GITHUB_TOKEN` | Yes | Yes (`re_42`, `re_43`, `re_403`–`re_405`) | READY* | Default repo search remains `GET /search/repositories`. Wave 1 adds explicit `github issues:` / `github prs:` query prefixes that use `GET /search/issues` with `is:issue` / `is:pr` qualifiers. No code search, commit search, release search, GraphQL, or write path. Capability: `search` only. |
